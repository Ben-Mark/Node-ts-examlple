...
function runTest(testName: string, dbType: string, port?: number) {
    ...
    describe(testName, function (this: Context) {
        ...
        let app: any
        let httpTestServer: any

        before(async function () {
            try {

                console.log('Setting up resources before all tests run.');
                const expressApp = await createApp(port)
                app = expressApp.app
                httpTestServer = expressApp.httpServer
        ...
        after(async function () {
            ...
            // Close
            await terminate(httpTestServer)
...
describe('Test suite', function () {
    ...
    //     runTest('CRUD Animal Test with mongoDB', dbType, 3007);
    // } else {
